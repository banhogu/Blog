...
<div className="bg-my-gray py-1 px-5 text-base font-medium rounded-xl dark:text-white dark:bg-gray-800">
  ...
  컴포넌트의 입맛에 맞게 바꿔야 하므로, API 변경 사항 하나로 전체 코드 구조가 달라진다는점에서
  결합도가 높은 상황이라고 판단했다.
</div>
...
변경 전 기존 Index를 코드를 보면 API 요청, 데이터 가공, 컴포넌트 결합 등 해당 컴포넌트는 어떤 역할의 컴포넌트인건지 알 수 없을것이다.
...
자신이 필요한 데이터 형식을 가지고 있는 Factory 함수에게 데이터를 요청하면 바로 사용 할 수 있게 되었다.
...
<div className="bg-my-gray py-1 px-5 text-base font-medium rounded-xl dark:text-white dark:bg-gray-800">
  ...
  형식으로 변경 되어도 코드 수정을 최소화 할 수 있고, 데이터 가공 또한 Factory 함수가 담당함으로써
  UI 컴포넌트의 코드를 더 간결하게 유지할 수 있다.
</div>
