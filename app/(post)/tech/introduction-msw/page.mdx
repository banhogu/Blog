...
그럼에도 시간이 부족하다고 판단하였고, **MSW** 도입과 **기존 컴포넌트에 레이어를 추가**하는 방식을 통해 해당 문제를 해결하고, API에 종속적인 프론트 개발 생산성을 증가시켰던 경험을 소개하고자 한다.
...
> 심지어 컴포넌트에서 받아온 가공된 Props를 직접 사용하는것이 아닌, 다시 컴포넌트 형식에 맞게 바꿔주는 작업 또한 필요했다.

<aside>
❗ Index Page에서는 단순히 컴포넌트를 결합하여 화면으로 보여주는 역할만 수행해야 하는데, API 요청, 데이터 가공, 컴포넌트 결합 등 책임이 많아졌고, Index Page에서의 가공된 Props에 따라서 다시 컴포입맛에 맞게 바꿔야 하므로, 전체 코드 구조가 달라진다는점에서 결합도가 높은 상황이다.
...
따라서 Index에서의 API 요청, 데이터 가공작업, 자식 컴포넌트의 코드 변경을 최소화하기 위해, API 데이터가 어떻게 내려오는지에 대한 관심사는 모두 Factory에게 넘겨주고, UI 컴포넌트에서는 Factory에서 화면을 그리는 데 필요한 데이터들만 받아서 바로 사용 할 수 있도록 변경했다.
...
API 요청, 데이터 가공, 컴포넌트 결합 등 대체 얘는 뭐하는 컴포넌트인가 싶다.
...
부모로 부터 받아온 props를 재가공하여, 불필요한 로직이 수행되고 있다.
...
자신이 필요한 데이터 형식을 가지고 있는 Factory 함수에게 데이터를 요청하면 가공할 필요 없이 바로 사용 할 수 있게 되었다.

<br />

<aside>
💡 API 응답 데이터가 확정되지 않은 상황에서 Factory 함수를 추가함으로써 UI 컴포넌트들을 API로부터 독립시킬 수 있게 되었다. 이렇게 개발된 UI 컴포넌트들은 예상과는 다른 API 데이터 형식으로 변경 되어도 Factory 함수에서 변경 작업을 통해 수정을 최소화할 수 있고, 테스트 코드를 일찌감치 확정 지을 수 있고, 분기 처리를 할 때에도 Factory가 담당하여 UI 컴포넌트를 좀 더 간결하게 유지할 수 있다.
...
> **src > Mocks > index.ts - 서버와 클라이언트 환경에 따라 API 모킹 설정을 초기화**
...
`isServer` 변수를 통해 현재 실행 환경이 서버인지 클라이언트인지 확인합니다. `typeof window === 'undefined'` 조건은 서버 환경 (Node.js)에서 `window` 객체가 존재하지 않음을 이용한 확인 방법!

<br />

> **src > Mocks > handlers.ts - API주소를 기반으로 만들어진 handlers를 하나로 통합**
...
> **src > Mocks > browser.ts, server.ts - 서버 및 클라이언트 환경에서 handlers 설정**
...
> **src > Mocks > postHandler > index.ts - 상세 글 API 요청에 대해 데이터 반환**
...
또한, MSW를 사용하여 API 응답 데이를을 가상으로 생성함으로써 프론트엔드 개발 과정에서 실제 API 호출의 필요성을 제거했다. 이를 통해 백엔드 개발이 완료되지 않은 상태에서도 프론트엔드 작업을 지속할 수 있었고, 각 팀이 독립적, 병렬적으로 작업을 진행할 수 있었다.
