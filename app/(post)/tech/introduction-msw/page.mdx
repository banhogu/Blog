...
<div className="bg-gray-100 py-1 px-2 rounded-xl dark:text-white dark:bg-gray-800">
...
<div className="bg-gray-100 py-1 px-2 rounded-xl dark:text-white dark:bg-gray-800">
  ...
  데이터 가공, 컴포넌트 결합 등 책임이 많아졌고, Index Page에서의 가공된 Props에 따라서 다시
  컴포입맛에 맞게 바꿔야 하므로, 전체 코드 구조가 달라진다는점에서 결합도가 높은 상황이다.
</div>
...
<div className="bg-gray-100 py-1 px-2 rounded-xl dark:text-white dark:bg-gray-800">
  💡 API 응답 데이터가 확정되지 않은 상황에서 Factory 함수를 추가함으로써 UI 컴포넌트들을 API로부터
  독립시킬 수 있게 되었다. 이렇게 개발된 UI 컴포넌트들은 예상과는 다른 API 데이터 형식으로 변경
  되어도 Factory 함수에서 변경 작업을 통해 수정을 최소화할 수 있고, 테스트 코드를 일찌감치 확정 지을
  수 있고, 분기 처리를 할 때에도 Factory가 담당하여 UI 컴포넌트를 좀 더 간결하게 유지할 수 있다.
</div>
