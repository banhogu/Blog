...
따라서 위와 같은 문제점을 해결하기 위해 Axios 추상화를 도입하기로 했다. <br /><br />
...
[에러 Custom Code 명세서 ](https://banhogudev.oopy.io/a072a6b2-ba87-4c70-b5ee-f72a905867d4) <br /><br />
...
- **timeout: 3000**
  - 요청이 취소되기 전에 기다리는 시간을 밀리초 단위로 설정. 여기서는 3000 밀리초(3초)로 설정되어 있는데 즉, 요청이 3초 안에 완료되지 않으면 요청이 타임아웃된다.
...
- **`config?: AxiosRequestConfig`**: 옵셔널한 요청 추가 설정. Axios의 요청 설정을 커스터마이즈할 때 사용한다.
